...
const PEOPLE_RESTRICTED_LIFECYCLE_CATEGORIES_0116D = new Set(["inactive", "suspended", "expelled", "former", "archived", "blocked"]);
const PEOPLE_BLOCKING_LIFECYCLE_CATEGORIES_0116D = new Set(["suspended", "expelled", "archived", "blocked"]);
...
async function peopleRequireDefinitionAdminAccess0116D(serviceClient: SupabaseClientAny, personId: string, organizationId: string, platformAdmin: boolean): Promise<JsonRecord> {
  if (platformAdmin) return (await buildPlatformAccess(serviceClient, organizationId))[0] || {};
  const row = await requireAnyMembershipAccess(serviceClient, personId, organizationId, ["people.manage_members", "people.manage_applicants", "access.manage_memberships", "organization.manage_settings", "organization.super_admin"]);
  if (!actorCanManagePeople(row) && !actorCanManageAccess(row)) throw new Error("You do not have permission to manage People definitions.");
  return row;
}

function sortedPeopleDefinitions0116D(rows: JsonRecord[]): JsonRecord[] {
...
async function peopleListDefinitions0116D(serviceClient: SupabaseClientAny, organizationId: string): Promise<JsonRecord> {
  ...
  const lifecycleStatuses = ((statuses.data || []) as JsonRecord[]).filter((status) => {
    ...
    return settings.legacy_mixed_status !== true && !["full-member", "probationary-member", "family-member", "honorary-member", "waitlist", "onboarding"].includes(statusKey);
  });
  return {
...
async function peopleSaveLifecycleStatus0116D(serviceClient: SupabaseClientAny, organizationId: string, body: JsonRecord): Promise<JsonRecord> {
  const id = clean(body.status_definition_id);
  const label = clean(body.label);
  if (!label) throw new Error("Lifecycle status name is required.");
  const existing = id ? (await serviceClient.from("core_membership_status_definitions").select("*").eq("organization_id", organizationId).eq("status_definition_id", id).maybeSingle()) : { data: null, error: null };
  if (existing.error) throw existing.error;
  const existingRow = (existing.data || {}) as JsonRecord;
  const statusKey = id ? clean(existingRow.status_key) : normalizeLifecycleStatusDefinitionKey0116D(body.status_key || label);
  if (!statusKey) throw new Error("Could not create a safe status key from that name.");
  const lifecycleCategory = normalizeLifecycleCategory0116D(body.lifecycle_category || existingRow.lifecycle_category || (normalizeKey(label) === "active" ? "active" : "prospect"));
  const statusValue = normalizeDefinitionStatus0116D(body.status || existingRow.status || "active");
  const restricted = PEOPLE_RESTRICTED_LIFECYCLE_CATEGORIES_0116D.has(lifecycleCategory) || statusValue === "archived";
  const blocking = PEOPLE_BLOCKING_LIFECYCLE_CATEGORIES_0116D.has(lifecycleCategory) || statusValue === "archived";
  ...
  const payload: JsonRecord = {
    ...
    is_active_member: !restricted && (optionalBoolean(body, "is_active_member", lifecycleCategory === "active") || lifecycleCategory === "active"),
    can_login: blocking ? false : optionalBoolean(body, "can_login", !restricted && ["active", "invited", "pending", "onboarding"].includes(lifecycleCategory)),
    can_view_member_portal: blocking ? false : optionalBoolean(body, "can_view_member_portal", !restricted && ["active", "invited", "pending", "onboarding"].includes(lifecycleCategory)),
    can_reserve_assets: restricted ? false : optionalBoolean(body, "can_reserve_assets", lifecycleCategory === "active"),
    requires_admin_review: optionalBoolean(body, "requires_admin_review", ["prospect", "applicant", "onboarding", "invited", "pending", "suspended", "expelled", "blocked"].includes(lifecycleCategory)),
  ...
  if (id) {
    const { data, error } = await serviceClient.from("core_membership_status_definitions").update(payload).eq("organization_id", organizationId).eq("status_definition_id", id).select("*").single();
    if (error) throw error;
    return data as JsonRecord;
  }
  const { data, error } = await serviceClient.from("core_membership_status_definitions").insert(payload).select("*").single();
  if (error) throw error;
  return data as JsonRecord;
}

async function peopleSetLifecycleStatusArchive0116D(serviceClient: SupabaseClientAny, organizationId: string, statusDefinitionId: string, archived: boolean): Promise<JsonRecord> {
  ...
  const { data, error } = await serviceClient.from("core_membership_status_definitions").update(payload).eq("organization_id", organizationId).eq("status_definition_id", statusDefinitionId).select("*").single();
  if (error) throw error;
  return data as JsonRecord;
}

async function peopleSaveMembershipClass0116D(serviceClient: SupabaseClientAny, organizationId: string, body: JsonRecord): Promise<JsonRecord> {
  const id = clean(body.membership_class_definition_id);
  const label = clean(body.label);
  if (!label) throw new Error("Membership class name is required.");
  const existing = id ? (await serviceClient.from("core_membership_class_definitions").select("*").eq("organization_id", organizationId).eq("membership_class_definition_id", id).maybeSingle()) : { data: null, error: null };
  if (existing.error) throw existing.error;
  const existingRow = (existing.data || {}) as JsonRecord;
  const classKey = id ? clean(existingRow.class_key) : normalizeKey(body.class_key || label);
  if (!classKey) throw new Error("Could not create a safe class key from that name.");
  const rawSettings = jsonObject(existingRow.settings_json);
  const statusValue = normalizeDefinitionStatus0116D(body.status || existingRow.status || "active");
  const payload: JsonRecord = {
    organization_id: organizationId,
    ...
    class_category: normalizeKey(body.class_category || existingRow.class_category || "member") || "member",
    dues_behavior: normalizeKey(body.dues_behavior || existingRow.dues_behavior || "standard") || "standard",
    billing_notes: clean(body.billing_notes || "") || null,
    privilege_notes: clean(body.privilege_notes || "") || null,
    default_can_reserve_assets: optionalBoolean(body, "default_can_reserve_assets", Boolean(existingRow.default_can_reserve_assets)),
    default_can_view_member_documents: optionalBoolean(body, "default_can_view_member_documents", existingRow.default_can_view_member_documents !== false),
    requires_admin_review: optionalBoolean(body, "requires_admin_review", Boolean(existingRow.requires_admin_review)),
    is_default: optionalBoolean(body, "is_default", Boolean(existingRow.is_default)),
    sort_order: Number(body.sort_order ?? existingRow.sort_order ?? 100) || 100,
    status: statusValue,
  ...
  if (id) {
    const { data, error } = await serviceClient.from("core_membership_class_definitions").update(payload).eq("organization_id", organizationId).eq("membership_class_definition_id", id).select("*").single();
    if (error) throw error;
    return data as JsonRecord;
  }
  const { data, error } = await serviceClient.from("core_membership_class_definitions").insert(payload).select("*").single();
  if (error) throw error;
  return data as JsonRecord;
}

async function peopleSetMembershipClassArchive0116D(serviceClient: SupabaseClientAny, organizationId: string, membershipClassDefinitionId: string, archived: boolean): Promise<JsonRecord> {
  const payload: JsonRecord = archived ? { archived_at: new Date().toISOString(), status: "archived", sort_order: 999 } : { archived_at: null, status: "active" };
  const { data, error } = await serviceClient.from("core_membership_class_definitions").update(payload).eq("organization_id", organizationId).eq("membership_class_definition_id", membershipClassDefinitionId).select("*").single();
  if (error) throw error;
  return data as JsonRecord;
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (action.startsWith("organization_")) {
      ...
      if (action === "organization_list_people_definitions") {
      ...
      if (action === "organization_save_lifecycle_status") {
        const actorAccess = await peopleRequireDefinitionAdminAccess0116D(serviceClient, personId, organizationId, platformAdmin);
        const lifecycle_status = await peopleSaveLifecycleStatus0116D(serviceClient, organizationId, body);
        await writeAudit(serviceClient, actorEmail, "organization_admin", action, "core_membership_status_definitions", clean(lifecycle_status.status_definition_id), { organization_id: organizationId, label: clean(body.label) }, { saved: true });
        const result = await peopleListDefinitions0116D(serviceClient, organizationId);
        return jsonResponse(200, { ok: true, action, access: actorAccess, lifecycle_status, ...result });
      }

      if (action === "organization_archive_lifecycle_status" || action === "organization_restore_lifecycle_status") {
        const actorAccess = await peopleRequireDefinitionAdminAccess0116D(serviceClient, personId, organizationId, platformAdmin);
        const statusDefinitionId = requireString(body, "status_definition_id");
        const lifecycle_status = await peopleSetLifecycleStatusArchive0116D(serviceClient, organizationId, statusDefinitionId, action === "organization_archive_lifecycle_status");
        await writeAudit(serviceClient, actorEmail, "organization_admin", action, "core_membership_status_definitions", statusDefinitionId, { organization_id: organizationId }, { archived: action === "organization_archive_lifecycle_status" });
        const result = await peopleListDefinitions0116D(serviceClient, organizationId);
        return jsonResponse(200, { ok: true, action, access: actorAccess, lifecycle_status, ...result });
      }

      if (action === "organization_reorder_lifecycle_statuses") {
        const actorAccess = await peopleRequireDefinitionAdminAccess0116D(serviceClient, personId, organizationId, platformAdmin);
        await peopleReorderDefinitions0116D(serviceClient, organizationId, "lifecycle_status", Array.isArray(body.status_definition_ids) ? body.status_definition_ids.map(clean) : []);
        await writeAudit(serviceClient, actorEmail, "organization_admin", action, "core_membership_status_definitions", organizationId, { organization_id: organizationId }, { reordered: true });
        const result = await peopleListDefinitions0116D(serviceClient, organizationId);
        return jsonResponse(200, { ok: true, action, access: actorAccess, ...result });
      }

      if (action === "organization_save_membership_class") {
        const actorAccess = await peopleRequireDefinitionAdminAccess0116D(serviceClient, personId, organizationId, platformAdmin);
        const membership_class = await peopleSaveMembershipClass0116D(serviceClient, organizationId, body);
        await writeAudit(serviceClient, actorEmail, "organization_admin", action, "core_membership_class_definitions", clean(membership_class.membership_class_definition_id), { organization_id: organizationId, label: clean(body.label) }, { saved: true });
        const result = await peopleListDefinitions0116D(serviceClient, organizationId);
        return jsonResponse(200, { ok: true, action, access: actorAccess, membership_class, ...result });
      }

      if (action === "organization_archive_membership_class" || action === "organization_restore_membership_class") {
        const actorAccess = await peopleRequireDefinitionAdminAccess0116D(serviceClient, personId, organizationId, platformAdmin);
        const membershipClassDefinitionId = requireString(body, "membership_class_definition_id");
        const membership_class = await peopleSetMembershipClassArchive0116D(serviceClient, organizationId, membershipClassDefinitionId, action === "organization_archive_membership_class");
        await writeAudit(serviceClient, actorEmail, "organization_admin", action, "core_membership_class_definitions", membershipClassDefinitionId, { organization_id: organizationId }, { archived: action === "organization_archive_membership_class" });
        const result = await peopleListDefinitions0116D(serviceClient, organizationId);
        return jsonResponse(200, { ok: true, action, access: actorAccess, membership_class, ...result });
      }

      if (action === "organization_reorder_membership_classes") {
        const actorAccess = await peopleRequireDefinitionAdminAccess0116D(serviceClient, personId, organizationId, platformAdmin);
