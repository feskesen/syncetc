...
(function () {
  ...
  function render() {
    ...
    </style><div class="login-card"><div class="brand"><div class="mark">S</div><div><h1>SyncEtc Login</h1><div class="help">One login for user access, organization admin, and platform tools.</div></div></div>${authenticated ? `<p><strong>Logged in as ${esc(email)}</strong></p><div class="actions"><a class="btn" href="/user-dashboard" style="text-decoration:none;display:inline-flex;align-items:center;justify-content:center">Open User Dashboard</a><a class="btn secondary" href="/organization-admin" style="text-decoration:none;display:inline-flex;align-items:center;justify-content:center">Open Organization Admin</a><button id="login-logout" class="btn secondary" type="button">Log out</button></div>` : `<div class="login-grid"><input id="login-email" type="email" placeholder="Email" autocomplete="username"><input id="login-password" type="password" placeholder="Password" autocomplete="current-password"><button id="login-submit" class="btn" type="button">Log in</button></div><div class="actions"><button id="login-reset" class="btn secondary" type="button">Send password reset</button></div>`}<div class="msg">${esc(message)}</div></div>`;
  ...
  document.addEventListener("DOMContentLoaded", () => refreshAuth().catch((e) => { message = e.message || String(e); messageKind = "warn"; render(); }));
})();
