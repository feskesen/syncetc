...
(function () {
  ...
  const VERSION = "2026-06-12-108-J";
  ...
  function styleVars() {
    ...
    const attention = colors.alert_danger || colors.danger || colors.error || colors.status_attention || "#b42318";
    const attentionBg = colors.alert_danger_bg || colors.danger_bg || colors.error_bg || colors.status_attention_bg || "#fee4e2";
    const warning = colors.alert_warning || colors.warning || colors.status_warning || "#b54708";
    const warningBg = colors.alert_warning_bg || colors.warning_bg || colors.status_warning_bg || "#fef0c7";
    return `--ap-primary:${esc(colors.brand_primary || "#265c2b")};--ap-soft:${esc(colors.brand_secondary || "#edf7ed")};--ap-text:${esc(colors.text || "#142417")};--ap-surface:${esc(colors.surface || "#fff")};--ap-attention:${esc(attention)};--ap-attention-bg:${esc(attentionBg)};--ap-warning:${esc(warning)};--ap-warning-bg:${esc(warningBg)};--ap-page-width:${width};`;
  }
...
.ap-wrap{max-width:var(--ap-page-width,1180px);margin:0 auto 48px;padding:16px;font-family:Arial,Helvetica,sans-serif;color:var(--ap-text,#142417)}.ap-wrap *{box-sizing:border-box}.ap-panel{background:var(--ap-surface,#fff);border:1px solid color-mix(in srgb,var(--ap-primary,#265c2b) 15%,#d9e2ec);border-radius:24px;box-shadow:0 14px 40px rgba(12,38,64,.14);overflow:hidden}.ap-hero{padding:24px;background:linear-gradient(135deg,var(--ap-primary,#265c2b),color-mix(in srgb,var(--ap-primary,#265c2b) 55%,#4c8f55));color:#fff}.ap-hero h1{margin:0;font-size:clamp(30px,4vw,48px);letter-spacing:-.035em}.ap-hero p{margin:9px 0 0;max-width:900px;color:rgba(255,255,255,.9);font-weight:800;line-height:1.45}.ap-body{padding:16px;background:color-mix(in srgb,var(--ap-soft,#edf7ed) 38%,#fff)}.ap-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px}.ap-card{background:#fff;border:1px solid color-mix(in srgb,var(--ap-primary,#265c2b) 14%,#d9e2ec);border-radius:18px;padding:14px}.ap-card h2,.ap-card h3{margin:0 0 8px;color:var(--ap-primary,#265c2b)}.ap-kicker{font-size:11px;font-weight:950;letter-spacing:.1em;text-transform:uppercase;opacity:.88}.ap-label{font-size:11px;text-transform:uppercase;letter-spacing:.05em;font-weight:950;color:var(--ap-primary,#265c2b)}.ap-field{display:grid;gap:5px;margin-top:10px}.ap-input,.ap-select,.ap-textarea{width:100%;padding:10px 11px;border:1px solid #c5d4e2;border-radius:10px;font:inherit;background:#fff;color:#111827}.ap-textarea{min-height:90px;resize:vertical}.ap-pill{display:inline-flex;align-items:center;border-radius:999px;padding:5px 9px;background:color-mix(in srgb,var(--ap-soft,#edf7ed) 82%,#fff);color:var(--ap-primary,#265c2b);font-weight:950;font-size:12px}.ap-pill.warn{background:var(--ap-warning-bg,#fef0c7);color:var(--ap-warning,#b54708)}.ap-pill.bad{background:var(--ap-attention-bg,#fee4e2);color:var(--ap-attention,#b42318)}.ap-alert{padding:10px 12px;border-radius:12px;font-weight:850;margin-bottom:10px}.ap-alert.ok{background:#e7f6ec;color:#17633a}.ap-alert.bad{background:#fee2e2;color:#991b1b}.ap-alert.info{background:#eaf5ff;color:#1f4f82}.ap-btn{border:1px solid var(--ap-primary,#265c2b);background:var(--ap-primary,#265c2b);color:#fff;border-radius:999px;padding:9px 13px;font-weight:900;cursor:pointer;transition:.12s;box-shadow:0 6px 14px rgba(12,38,64,.12)}.ap-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 9px 18px rgba(12,38,64,.18)}.ap-btn.secondary{background:#fff;color:var(--ap-primary,#265c2b)}.ap-btn:disabled{opacity:.55;cursor:not-allowed;transform:none;box-shadow:none}.ap-actions{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:12px}.ap-status-block{margin-top:2px;color:#52606d;font-size:13px;line-height:1.45;display:grid;gap:8px}.ap-status-title{font-size:18px;font-weight:950;color:var(--ap-primary,#265c2b)}.ap-status-line{display:grid;grid-template-columns:minmax(0,160px) minmax(0,1fr);gap:10px;align-items:baseline}.ap-status-line span{color:#697586}.ap-status-line strong{color:#1f2937}.ap-status-pills{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.ap-progress{display:grid;gap:8px;margin-top:12px}.ap-progress-row{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center}.ap-bar{height:10px;border-radius:999px;background:#e5edf4;overflow:hidden}.ap-bar span{display:block;height:100%;background:var(--ap-primary,#265c2b);border-radius:999px}.ap-accordion{margin-top:14px;display:grid;gap:10px}.ap-section{border:1px solid #dbe5ee;border-radius:16px;background:#fff;overflow:hidden}.ap-section summary{list-style:none;cursor:pointer;padding:14px 15px;font-weight:950;color:var(--ap-primary,#265c2b);display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:10px;align-items:center}.ap-section summary::-webkit-details-marker{display:none}.ap-section summary:after{content:'▾';font-size:14px;transition:.12s;justify-self:end}.ap-section[open] summary:after{transform:rotate(180deg)}.ap-section-body{padding:0 14px 14px}.ap-task{border:1px solid #e1e8f0;border-radius:14px;padding:12px;margin-top:10px;background:#fff}.ap-task-head{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.ap-task-title{font-weight:950;color:#172033}.ap-task-desc{font-size:13px;color:#52606d;margin-top:3px;line-height:1.4}.ap-upload{margin-top:10px;padding:10px;border:1px dashed #bfd0df;border-radius:12px;background:#fbfdff}.ap-upload-row{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}.ap-upload-list{margin-top:8px;display:grid;gap:6px}.ap-upload-item{font-size:13px;padding:8px 10px;border-radius:10px;background:#f8fafc;border:1px solid #e5edf5}.ap-muted{color:#52606d;font-size:13px;line-height:1.45}.ap-debug{margin-top:12px;padding:12px;border-radius:12px;background:#101828;color:#dbeafe;font:12px ui-monospace,Menlo,Consolas,monospace;white-space:pre-wrap;overflow:auto}.ap-note{font-size:13px;color:#52606d}.ap-hidden{position:absolute!important;left:-9999px!important;width:1px!important;height:1px!important;overflow:hidden!important}@media(max-width:850px){.ap-grid{grid-template-columns:1fr}.ap-upload-row{grid-template-columns:1fr}.ap-progress-row{grid-template-columns:1fr}.ap-wrap{padding:10px}}`; }
  ...
  function applicantStatusCopy(app) {
    ...
    const map = {
      ...
      converted: "Membership created",
    };
    return map[key] || clean(app.status_label || app.status || "Application status");
  }
  function completedTask(task) {
    return ["completed","waived","accepted"].includes(normalize(task.status || task.review_status || task.upload_status));
  }
  function statusHtml(app) {
    const label = applicantStatusCopy(app);
    const submitted = fmtDate(app.submitted_at || app.created_at);
    const updated = fmtDateTime(app.last_applicant_update_at);
    const waitlist = app.waitlist_order ? `<div class="ap-status-line"><span>Waitlist position</span><strong>${esc(app.waitlist_order)}</strong></div>` : "";
    return `<div class="ap-status-block"><div class="ap-status-title">${esc(label)}</div>${app.display_name?`<div class="ap-status-line"><span>Application for</span><strong>${esc(app.display_name)}</strong></div>`:""}${submitted?`<div class="ap-status-line"><span>Submitted</span><strong>${esc(submitted)}</strong></div>`:""}${updated?`<div class="ap-status-line"><span>Last updated</span><strong>${esc(updated)}</strong></div>`:""}${waitlist}</div>`;
  }
  ...
  function loginRequestHtml() { const org = state.organization || {}; const title = org.display_name || 'Applicant Portal'; const email = state.requestEmail || state.email || ''; return `<style>${css()}</style><div class="ap-wrap" style="${styleVars()}"><section class="ap-panel"><div class="ap-hero"><div class="ap-kicker">Applicant Portal</div><h1>${esc(title)}</h1><p>Use this page after you receive a secure applicant portal link, or enter your application email below to request a new link.</p></div><div class="ap-body"><div class="ap-card"><h2>Access your applicant portal</h2><p class="ap-muted">Enter the email address you used when you applied. We will send a secure link you can use to view or update your application information and complete any requested next steps.</p>${state.requestMessage?`<div class="ap-alert ${state.requestKind||'info'}">${esc(state.requestMessage)}</div>`:''}<label class="ap-field"><span class="ap-label">Application email</span><input class="ap-input" id="ap-request-email" type="email" autocomplete="email" inputmode="email" value="${attr(email)}" placeholder="email@example.com"></label><div class="ap-actions"><button class="ap-btn" id="ap-request-link" ${state.requestBusy?'disabled':''}>${state.requestBusy?'Sending…':'Send secure link'}</button><a class="ap-btn secondary" href="/apply-now">Begin application</a><a class="ap-btn secondary" href="/contact">Contact us</a></div><p class="ap-muted">If you have not applied yet, begin the application process. If you no longer have access to the email address on file, contact the organization so it can help verify and update your record.</p></div>${DEBUG?`<pre class="ap-debug">SyncEtc Applicant Portal ${VERSION}\nLogged out/request mode\nOrg: ${esc(org.organization_key||rootData().organizationKey)}\nSteps:\n${esc(steps.join("\n"))}\n\n${esc(JSON.stringify(state.payload,null,2))}</pre>`:''}</div></section></div>`; }

  function html() {
    const app = state.applicant;
    const org = state.organization || {};
    const orgName = org.display_name || 'the organization';
    if (state.loading) return `<style>${css()}</style><div class="ap-wrap" style="${styleVars()}"><div class="ap-panel"><div class="ap-body">${esc(state.authCallbackMessage || 'Loading applicant portal…')}</div></div></div>`;
    if (state.error) return loginRequestHtml();
    if (state.loggedOut || !app) return loginRequestHtml();
    const nextSteps = applicantVisibleTasks();
    const summary = taskSummary(nextSteps);
    const progress = requiredTaskProgress();
    const statusLabel = applicantStatusCopy(app);
    const statusOpen = state.openSection === 'status' || !state.openSection;
    const tasksOpen = state.openSection === 'tasks';
    const applicationOpen = state.openSection === 'application';
    return `<style>${css()}</style><div class="ap-wrap" style="${styleVars()}"><section class="ap-panel"><div class="ap-hero"><div class="ap-kicker">Applicant Portal</div><h1>${esc(org.display_name || 'Applicant Portal')}</h1><p>View your application status, update your application information, and complete any steps requested by ${esc(orgName)}.</p></div><div class="ap-body">${state.message?`<div class="ap-alert ${state.messageKind || 'ok'}">${esc(state.message)}</div>`:""}<div class="ap-accordion"><details class="ap-section" ${statusOpen?'open':''} data-section="status"><summary><span>Application status</span><span class="ap-pill">${esc(statusLabel)}</span></summary><div class="ap-section-body">${statusHtml(app)}</div></details><details class="ap-section" ${tasksOpen?'open':''} data-section="tasks"><summary><span>Next steps for you</span><span class="ap-pill ${summary.className}">${esc(summary.label)}</span></summary><div class="ap-section-body">${progress.total?`<div class="ap-progress"><div class="ap-progress-row"><strong>Required items</strong><span>${progress.done} of ${progress.total} complete</span></div><div class="ap-bar"><span style="width:${progress.pct}%"></span></div></div>`:""}${nextSteps.length?`<p class="ap-muted">Complete the items listed below.</p>${nextSteps.map(taskHtml).join("")}`:'<div class="ap-muted">No action is required from you right now.</div>'}</div></details><details class="ap-section" ${applicationOpen?'open':''} data-section="application"><summary><span>Update application information</span><span class="ap-pill">${app.can_update === false ? 'View only' : 'Editable'}</span></summary><div class="ap-section-body">${app.can_update === false ? '<p class="ap-muted">Application updates are not currently enabled for this stage.</p>' : '<p class="ap-muted">Review and update your application information below. Use Save application updates before leaving this page.</p>'}${applicationForm(app)}</div></details></div>${DEBUG?`<pre class="ap-debug">SyncEtc Applicant Portal ${VERSION}\nElapsed: ${Date.now()-startMs}ms\nEmail: ${esc(state.email)}\nApplicant-facing next steps: ${nextSteps.length}\nSteps:\n${esc(steps.join("\n"))}\n\n${esc(JSON.stringify(state.payload,null,2))}</pre>`:""}</div></section></div>`;
  }
  ...
  async function saveUpdate() { if (state.saving) return; state.saving = true; const note=byId('ap-save-note'); if(note) note.textContent='Saving…'; try { const data = await accessCall({ action:'applicant_save_my_application', ...collectUpdate() }); state.payload=data; state.applicant=data.application || data.applicant; state.settings=obj(data.settings); state.message='Application information updated.'; state.messageKind='ok'; state.dirty=false; render(); } catch(error){ if(note) note.textContent=''; state.message=readableError(error, 'The update could not be saved.'); state.messageKind='bad'; render(); } finally { state.saving=false; } }
  ...
  function bind() { const req=byId('ap-request-link'); if(req) req.onclick=requestPortalLink; const reqEmail=byId('ap-request-email'); if(reqEmail) reqEmail.addEventListener('input',e=>{state.requestEmail=e.target.value||'';}); const save=byId('ap-save'); if(save) save.onclick=saveUpdate; document.querySelectorAll('.ap-upload-btn').forEach(btn=>btn.addEventListener('click',()=>uploadTask(btn.dataset.taskId))); document.querySelectorAll('.ap-input,.ap-textarea,.ap-select').forEach(el=>el.addEventListener('input',()=>{ if(el.id !== 'ap-request-email') state.dirty=true;})); document.querySelectorAll('details[data-section]').forEach(el=>el.addEventListener('toggle',()=>{ if(el.open) state.openSection = el.dataset.section || state.openSection; })); }
  function bindNavAway() { window.addEventListener('beforeunload',(event)=>{ if(!state.dirty) return; event.preventDefault(); event.returnValue=''; }); document.addEventListener('click',(event)=>{ if(!state.dirty) return; const link = event.target && event.target.closest ? event.target.closest('a[href]') : null; if(!link) return; const href = link.getAttribute('href') || ''; if(!href || href.startsWith('#') || href.startsWith('javascript:')) return; if(!window.confirm('You have unsaved application updates. Leave this page?')) { event.preventDefault(); event.stopPropagation(); } }, true); }
