...
(function () {
  ...
  function styleVars() {
    ...
    return `--rsvp-brand:${e(brand)};--rsvp-soft:${e(soft)};--rsvp-text:${e(text)};--rsvp-surface:${e(surface)};`;
  }
...
`;
  ...
  function render() {
    ...
    root.innerHTML = `<style>${css()}</style><section class="rsvp-page" style="${styleVars()}"><div class="rsvp-shell"><header class="rsvp-hero"><div class="rsvp-pill" style="background:rgba(255,255,255,.18);color:#fff">Club Event RSVP</div><h1>Submit or update your RSVP</h1><p>Confirm the event below, update your RSVP, and see who else is coming.</p></header><main class="rsvp-main"><div style="text-align:right;margin-bottom:10px"><a id="rsvp-return-calendar" class="rsvp-btn secondary" href="/calendar">Return to Calendar</a></div>${eventHeader(ev)}${formHtml(ev)}${summaryHtml()}${DEBUG ? `<pre class="rsvp-debug">RSVP ${VERSION}\nMode: ${e(state.mode)}\nEvent: ${e(ev.event_id)}\nCan RSVP: ${e(ev.viewer_can_rsvp || ev.public_can_rsvp || ev.can_rsvp)}\nChecklist items: ${e(checklistItems().length)}</pre>` : ""}</main></div></section>`;
    bind();
    updateTotal();
  }
  ...
  function bind() {
    ...
    const ret = document.getElementById("rsvp-return-calendar");
    if (ret) ret.addEventListener("click", function (event) { if (!confirmLeaveIfDirty()) event.preventDefault(); });
    updateSaveUi();
  }
  ...
  async function saveRsvp() {
    ...
    try {
      ...
      setTimeout(() => { state.savedFlash = false; updateSaveUi(); }, 1800);
    } catch (err) {
