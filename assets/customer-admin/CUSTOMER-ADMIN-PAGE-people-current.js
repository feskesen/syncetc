...
(function () {
  ...
  function filteredPeople() {
    const rows = people.filter((p) => personMatchesLens(p, filter) && personMatchesSearch(p, search));
  ...
  function renderPersonCard(p) {
    ...
    const badges = [p.lifecycle_status_label || p.lifecycle_status_key, p.membership_class_label, hasSuperAdminRoleKeys(p) ? "Super Admin" : hasAnyRole(p, ["organization-admin"]) ? "Admin" : arr(p.role_keys).some(isManagerRole) ? "Manager" : hasRole(p, "board-member") ? "Board" : "", p.login_linked ? "Login" : "No login"].filter(Boolean).slice(0, 4);
  ...
  function renderAccessTab(row, roles, mayEdit, mayEditAnyRole, mayEditSuperAdmin) {
    ...
    const accessSummary = `<div class="people-access-summary">
      ...
      <div><span>Roles</span><strong>${esc(roleLabels.length ? roleLabels.join(", ") : "No roles assigned")}</strong></div>
    </div>`;
    return `<div class="people-access-callout"><div><strong>Access actions</strong><p>Send login help or update organization roles for this person.</p></div><div class="people-inline-actions"><button id="people-invite" class="people-btn secondary" type="button" ${mayEdit ? "" : "disabled"}>Send invite</button><button id="people-reset-password" class="people-btn secondary" type="button" ${mayEdit ? "" : "disabled"}>Password reset</button></div></div>${accessSummary}${ownWarning}<p class="muted">Assign only the access this person needs. Super Admin should be limited to trusted organization owners.</p><div class="people-check-grid">${roles.map((role) => { const rk = key(role.role_key); const locked = !mayEditAnyRole || (isSuperAdminRole(rk) && !mayEditSuperAdmin); const hint = locked && isSuperAdminRole(rk) ? "Super Admin locked" : ""; return checkbox(`role-${rk}`, role.label || rk, rowRoleKeys.includes(rk), locked, hint); }).join("")}</div>${!mayEditAnyRole ? `<p class="people-warning">Role editing is locked for your account.</p>` : !mayEditSuperAdmin ? `<p class="people-warning">Organization Super Admin is locked. You can manage ordinary roles and Organization Admin.</p>` : ""}`;
  }
