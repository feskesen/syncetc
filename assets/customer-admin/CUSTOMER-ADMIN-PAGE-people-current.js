...
(function () {
  ...
  function renderReservationReadiness0116R(row = selected || {}) {
    const readiness = reservationReadiness0116R(row);
    if (!readiness.required.length) return "";
    const problems = readiness.missing.concat(readiness.expired);
    if (!problems.length) {
      return `<div class="people-readiness-card ok"><strong>Reservation readiness</strong><span>Ready to reserve</span><small>All required qualifications are current.</small></div>`;
    }
    const details = [readiness.missing.length ? `Missing: ${readiness.missing.join(", ")}` : "", readiness.expired.length ? `Expired or needs date: ${readiness.expired.join(", ")}` : ""].filter(Boolean).join(" • ");
    return `<div class="people-readiness-card warn"><strong>Reservation readiness</strong><span>Needs attention</span><small>${esc(details)}</small></div>`;
  }
  ...
  function renderQualificationAssignmentRows(row, mayEdit) {
    ...
    return `<section class="people-qualification-panel people-qualification-grid-panel"><div class="people-role-section-head"><h4>Qualifications</h4></div>${renderReservationReadiness0116R(row)}<div class="people-qualification-table-wrap" tabindex="0" aria-label="Qualification fields"><div class="people-qualification-table" role="table"><div class="people-qualification-table-row people-qualification-table-head" role="row"><div role="columnheader">Qualification</div><div role="columnheader">Status / value</div><div role="columnheader">Completed / issued</div><div role="columnheader">Expires / current through</div><div role="columnheader">Notes</div></div>${definitions.map((definition) => {
      const id = qualificationDefinitionId(definition);
      ...
      return `<div class="people-qualification-table-row" role="row" data-qualification-field="${esc(domId)}" data-qualification-definition-id="${esc(id)}" data-qualification-key="${esc(qualificationDefinitionKey(definition))}" data-field-style="${esc(fieldStyle)}" data-assignment-id="${esc(clean(assignment.person_qualification_assignment_id || assignment.qualification_assignment_id || ""))}">
        <div class="people-qualification-title" role="cell"><strong>${esc(qualificationLabel(definition))}</strong>${definition.description ? `<small>${esc(definition.description)}</small>` : ""}${settings.required_to_reserve ? `<em>Required to reserve</em>` : ""}</div>
