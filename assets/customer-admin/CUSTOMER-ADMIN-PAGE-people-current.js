...
(function () {
  ...
  function isEndedStatus(statusKey) { return ["inactive", "former", "expelled", "archived", "suspended"].includes(key(statusKey)); }
  ...
  function updateAffiliationEndState() {
    const enabled = isEndedStatus($("people-status-key")?.value) || clean($("people-affiliation-end-date")?.value);
    ...
    if (hint) hint.textContent = enabled ? "Use when this organization affiliation has ended." : "Enabled when status is inactive, former, suspended, expelled, or archived.";
  }
  ...
  function bindEvents() {
    ...
    document.querySelectorAll(".people-editor input, .people-editor select, .people-editor textarea").forEach((el) => {
      ...
      el.addEventListener("change", () => { setDirty(true); updateAffiliationEndState(); refreshDisplayNamePreview(); });
    });
  ...
  function readForm() {
    ...
    const endEnabled = isEndedStatus(statusKey);
    const profile = {
