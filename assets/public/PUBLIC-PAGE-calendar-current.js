...
// Internal Version: 2026-06-09-093-A
// Purpose: Calendar / Events renderer with List, Compact, and Month views; public-safe event images/summaries; active control states; RSVP-preserving event detail modal.

(function () {
  "use strict";

  const VERSION = "2026-06-09-093-A";
  ...
  function monthEventPart(ev, dayDate) {
    const s = dayStartFromValue(ev.starts_at);
    const eDay = eventEndDay(ev);
    if (!s || !eDay || sameDay(s, eDay)) return "single";
    if (sameDay(s, dayDate)) return "start";
    if (sameDay(eDay, dayDate)) return "end";
    return "middle";
  }
  ...
  function monthViewHtml() {
    ...
    return `<section class="syncetc-month-shell"><div class="syncetc-month-head"><h2 class="syncetc-month-title">${e(monthTitle(cursor))}</h2><div class="syncetc-month-controls"><button class="syncetc-month-nav" data-month-nav="prev">← Previous</button><button class="syncetc-month-nav" data-month-nav="today">This Month</button><button class="syncetc-month-nav" data-month-nav="next">Next →</button></div></div><div class="syncetc-month-grid">${dows.map((d) => `<div class="syncetc-month-dow">${e(d)}</div>`).join("")}${days.map((d) => {
      ...
      const dayEvents = eventsForMonthDay(d).slice(0, 4);
      const extra = Math.max(0, eventsForMonthDay(d).length - dayEvents.length);
      return `<div class="syncetc-month-day ${inMonth ? "" : "outside"} ${sameDay(d, new Date()) ? "today" : ""}"><div class="syncetc-month-num">${d.getDate()}</div><div class="syncetc-month-events">${dayEvents.map((ev) => {
        const accent = eventAccent(ev, cfg);
        const part = monthEventPart(ev, d);
        const time = fmtTime(ev.starts_at);
        return `<button type="button" class="syncetc-month-event ${attr(part)}" style="--event-accent:${attr(accent)}" data-event-id="${attr(ev.event_id)}"><b>${e(ev.title)}</b>${time ? `<small>${e(time)}</small>` : ""}</button>`;
      }).join("")}${extra ? `<div class="syncetc-month-more">+${extra} more</div>` : ""}</div></div>`;
    }).join("")}</div></section>`;
  }
