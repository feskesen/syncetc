...
(function () {
  ...
  function buildCss(config) {
    return `
      ...
      .syncetc-docs-groups{display:grid;gap:18px}.syncetc-docs-group{padding:0;border:1px solid ${config.border};border-radius:${config.radius};background:${rgba(config.surface,.94)};overflow:hidden}.syncetc-docs-group summary{cursor:pointer;list-style:none;padding:${config.cardPadding};color:${config.primary};font-size:22px;font-weight:900}.syncetc-docs-group summary::-webkit-details-marker{display:none}.syncetc-docs-group summary span{float:right;font-size:12px;border:1px solid ${config.border};border-radius:999px;padding:4px 9px;color:${config.muted};background:${rgba(config.primary,.06)}}.syncetc-docs-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(245px,1fr));gap:14px;padding:0 ${config.cardPadding} ${config.cardPadding}}
      .syncetc-doc-card{display:grid;grid-template-columns:minmax(180px,.72fr) minmax(0,1fr);gap:16px;align-items:stretch;min-height:220px;padding:18px;border:1px solid ${config.border};border-radius:${config.radius};background:#fff;box-shadow:0 8px 20px rgba(12,38,64,.08)}.syncetc-doc-preview{min-height:220px;border:1px solid ${config.border};border-radius:14px;background:${rgba(config.primary,.04)};overflow:hidden;display:flex;align-items:center;justify-content:center}.syncetc-doc-preview iframe{width:100%;height:100%;min-height:220px;border:0;background:#fff}.syncetc-doc-preview-fallback{padding:14px;color:${config.muted};font-size:13px;line-height:1.45;text-align:center;font-weight:800}.syncetc-doc-card-content{display:grid;gap:14px;align-content:space-between}.syncetc-doc-card h3{margin:0 0 8px;color:${config.text};font-size:19px;line-height:1.15}.syncetc-doc-card p{margin:0;color:${config.muted};font-size:14px;line-height:1.55}.syncetc-doc-meta{margin-top:10px;color:${config.muted};font-size:12px;line-height:1.4}.syncetc-doc-actions{display:flex;gap:8px;flex-wrap:wrap}.syncetc-doc-button{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:8px 14px;border-radius:999px;background:${config.primary};color:#fff!important;text-decoration:none;font-weight:900;font-size:13px;white-space:nowrap;border:1px solid ${config.primary};cursor:pointer;transition:transform .16s ease,box-shadow .16s ease,background .16s ease}.syncetc-doc-button:hover{transform:translateY(-1px);box-shadow:0 8px 18px rgba(12,38,64,.16)}.syncetc-doc-button.secondary{background:#fff;color:${config.primary}!important;border-color:${config.border}}
      .syncetc-docs-note{margin-top:20px;padding:16px 18px;border-radius:${config.radius};border:1px solid ${config.border};background:${rgba(config.primary,.06)};color:${config.muted};font-size:13px;line-height:1.55}.syncetc-docs-empty{padding:22px;border:1px dashed ${config.border};border-radius:${config.radius};background:#fff;color:${config.muted};text-align:center;font-weight:800}.syncetc-doc-modal-backdrop{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(7,24,42,.72);z-index:2147483000;padding:24px}.syncetc-doc-modal-backdrop.is-open{display:flex}.syncetc-doc-modal{width:min(1100px,96vw);height:min(820px,92vh);background:#fff;border-radius:18px;box-shadow:0 24px 80px rgba(0,0,0,.38);display:grid;grid-template-rows:auto minmax(0,1fr);overflow:hidden}.syncetc-doc-modal-head{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:12px 14px;border-bottom:1px solid ${config.border};background:${rgba(config.primary,.06)}}.syncetc-doc-modal-frame{width:100%;height:100%;border:0;background:#fff}@media(max-width:920px){.syncetc-doc-card{grid-template-columns:1fr}.syncetc-doc-preview,.syncetc-doc-preview iframe{min-height:260px}}@media(max-width:720px){.syncetc-docs-main,.syncetc-docs-hero{padding:20px}.syncetc-doc-actions{display:grid}.syncetc-doc-button{width:100%}.syncetc-docs-list{grid-template-columns:1fr}}
    `;
  }
        ...
        <div class="syncetc-docs-shell">
          ...
          <main class="syncetc-docs-main">
            ...
            ${categories.length ? `<div class="syncetc-docs-groups">${categories.map((category) => `<details class="syncetc-docs-group" open><summary>${escapeHtml(category)} <span>${grouped[category].length}</span></summary><div class="syncetc-docs-list">${grouped[category].map((doc) => {
              ...
              return `<article class="syncetc-doc-card"><div class="syncetc-doc-preview">${inlinePreviewUrl ? `<iframe src="${escapeHtml(inlinePreviewUrl)}" title="${escapeHtml(doc.title || doc.original_file_name || "Document")} PDF preview"></iframe>` : `<div class="syncetc-doc-preview-fallback">PDF preview unavailable. Use Download PDF.</div>`}</div><div class="syncetc-doc-card-content"><div><h3>${escapeHtml(doc.title || doc.original_file_name || "Document")}</h3>${hasText(doc.description) ? `<p>${escapeHtml(doc.description)}</p>` : ""}<div class="syncetc-doc-meta">${escapeHtml(documentMeta(doc))}</div></div><div class="syncetc-doc-actions">${previewUrl ? `<button type="button" class="syncetc-doc-button" data-doc-preview="${escapeHtml(previewUrl)}" data-doc-title="${escapeHtml(doc.title || doc.original_file_name || "Document")}">Open Larger Preview</button>` : ""}${downloadUrl ? `<a class="syncetc-doc-button secondary" href="${escapeHtml(downloadUrl)}" target="_blank" rel="noopener">Download PDF</a>` : `<span class="syncetc-doc-meta">Unavailable</span>`}</div></div></article>`;
