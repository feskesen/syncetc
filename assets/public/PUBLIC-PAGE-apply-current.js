...
(function () {
  ...
  const state = { startedAt: performance.now(), steps: [], payload: null, submitting: false, submitted: false, error: "", backend: null, formStartedAt: Date.now() };
  ...
  function css() { const c = colors(); return `
    .syncetc-apply{max-width:1080px;margin:24px auto 56px;padding:0 18px;font-family:Arial,Helvetica,sans-serif;color:${c.text}}.syncetc-apply *{box-sizing:border-box}.syncetc-apply-card{background:rgba(255,255,255,.95);border:1px solid color-mix(in srgb,${c.primary} 16%,transparent);border-radius:26px;box-shadow:0 14px 42px rgba(12,38,64,.14);overflow:hidden}.syncetc-apply-hero{padding:30px;background:linear-gradient(135deg,${c.primary},color-mix(in srgb,${c.primary} 78%,#fff 22%));color:#fff}.syncetc-apply-eyebrow{display:inline-flex;margin-bottom:10px;padding:6px 12px;border-radius:999px;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.25);font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.08em}.syncetc-apply-hero h1{margin:0;color:#fff;font-size:clamp(32px,4vw,52px);line-height:1}.syncetc-apply-hero p{max-width:820px;margin:12px 0 0;color:rgba(255,255,255,.9);line-height:1.6}.syncetc-apply-body{padding:22px;background:linear-gradient(180deg,${c.secondary},rgba(255,255,255,.86))}.syncetc-section{margin:0 0 16px;padding:16px;border-radius:20px;background:#fff;border:1px solid color-mix(in srgb,${c.primary} 14%,transparent);box-shadow:0 8px 20px rgba(12,38,64,.07)}.syncetc-section h2{margin:0 0 6px;color:${c.primary};font-size:20px}.syncetc-section p{margin:0 0 12px;color:rgba(23,32,51,.68);line-height:1.5;font-size:13px}.syncetc-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.syncetc-grid-3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.syncetc-field{display:grid;gap:5px}.syncetc-field span{font-size:12px;color:${c.primary};font-weight:950}.syncetc-input,.syncetc-select,.syncetc-textarea{width:100%;padding:10px 12px;border-radius:13px;border:1px solid color-mix(in srgb,${c.primary} 18%,transparent);font:14px/1.4 Arial,Helvetica,sans-serif;color:${c.text};background:#fff}.syncetc-textarea{min-height:108px;resize:vertical}.syncetc-input:focus,.syncetc-select:focus,.syncetc-textarea:focus{outline:none;border-color:${c.primary};box-shadow:0 0 0 3px color-mix(in srgb,${c.primary} 13%,transparent)}.syncetc-required{color:#991b1b}.syncetc-actions{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin-top:16px}.syncetc-btn{min-height:40px;padding:10px 18px;border-radius:999px;border:1px solid ${c.primary};background:${c.primary};color:#fff;font-weight:950;cursor:pointer}.syncetc-btn:disabled{opacity:.55;cursor:not-allowed}.syncetc-alert{margin:12px 0 0;padding:12px 14px;border-radius:15px;border:1px solid color-mix(in srgb,${c.primary} 18%,transparent);background:#fff;font-weight:800;line-height:1.45}.syncetc-alert.error{background:#fee2e2;color:#991b1b;border-color:#fecaca}.syncetc-alert.ok{background:#e7f6ec;color:#166534;border-color:#bbf7d0}.syncetc-hidden{position:absolute;left:-9999px;opacity:0}.syncetc-debug{margin-top:14px;padding:14px;border-radius:18px;background:#0f172a;color:#dbeafe;font:12px/1.45 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;white-space:pre-wrap;overflow:auto}@media(max-width:760px){.syncetc-apply{padding:0 10px}.syncetc-grid,.syncetc-grid-3{grid-template-columns:1fr}.syncetc-apply-body{padding:14px}.syncetc-actions{justify-content:stretch}.syncetc-btn{width:100%}}
  `; }

  function field(name, label, type = "text", required = false, extra = "") { return `<label class="syncetc-field"><span>${esc(label)}${required ? ' <b class="syncetc-required">*</b>' : ''}</span><input class="syncetc-input" id="app-${name}" name="${esc(name)}" type="${esc(type)}" ${required ? "required" : ""} ${extra}></label>`; }
  function textarea(name, label, required = false, ph = "") { return `<label class="syncetc-field"><span>${esc(label)}${required ? ' <b class="syncetc-required">*</b>' : ''}</span><textarea class="syncetc-textarea" id="app-${name}" name="${esc(name)}" ${required ? "required" : ""} placeholder="${esc(ph)}"></textarea></label>`; }
  function select(name, label, opts, required = false) { return `<label class="syncetc-field"><span>${esc(label)}${required ? ' <b class="syncetc-required">*</b>' : ''}</span><select class="syncetc-select" id="app-${name}" name="${esc(name)}" ${required ? "required" : ""}>${opts.map(([v,l])=>`<option value="${esc(v)}">${esc(l)}</option>`).join("")}</select></label>`; }
  ...
  function formHtml() {
    ...
    return `<div class="syncetc-apply"><section class="syncetc-apply-card"><div class="syncetc-apply-hero"><div class="syncetc-apply-eyebrow">Application</div><h1>${esc(title)}</h1><p>${esc(intro)}</p></div><form id="syncetc-apply-form" class="syncetc-apply-body"><input class="syncetc-hidden" name="website" autocomplete="off"><input type="hidden" name="form_elapsed_ms" id="app-form-elapsed"><section class="syncetc-section"><h2>Basic information</h2><p>Start with your identity and contact details.</p><div class="syncetc-grid">${field("first_name","First name", "text", true)}${field("last_name","Last name", "text", true)}${field("date_of_birth","Date of birth", "date", true)}${field("email","Email", "email", true)}${field("phone","Mobile phone", "tel", true)}${field("home_phone","Home phone", "tel", false)}</div></section><section class="syncetc-section"><h2>Address and background</h2><div class="syncetc-grid">${field("address_1","Street address", "text", true)}${field("address_2","Address line 2")}${field("city","City", "text", true)}${field("state","State", "text", true)}${field("zip","ZIP", "text", true)}${field("employer","Employer")}${field("occupation","Occupation")}</div></section><section class="syncetc-section"><h2>Aviation qualifications</h2><p>This default aviation form can be customized later by organization.</p><div class="syncetc-grid">${field("pilot_certificate_number","Pilot certificate number", "text", true)}${select("certificate_level","Certificate level", [["private","Private Pilot"],["commercial","Commercial"],["atp","ATP"],["student","Student"],["other","Other"]], true)}${field("ratings","Ratings / endorsements")}${field("medical_class","Medical / BasicMed status")}${field("last_medical_date","Last medical date", "date")}${field("total_hours","Total flight hours")}${field("night_hours","Night hours")}${field("ifr_hours","IFR hours")}${field("complex_hours","Complex hours")}</div><div style="margin-top:12px">${textarea("aircraft_experience","Aircraft types and approximate hours", false, "Example: C172, PA28, complex aircraft, tailwheel, etc.")}${textarea("last_bfr","Last BFR / checkride details", false, "Aircraft, instructor/examiner, and date.")}</div></section><section class="syncetc-section"><h2>Safety and review questions</h2><div class="syncetc-grid">${select("accident_history","Aircraft accident / incident history", [["no","No"],["yes","Yes"]], true)}${select("faa_history","FAA action history", [["no","No"],["yes","Yes"],["pending","Pending / unsure"]], true)}</div><div class="syncetc-grid" style="margin-top:12px">${textarea("accident_details","Accident / incident details")}${textarea("faa_details","FAA action details")}</div></section><section class="syncetc-section"><h2>Interest and referral</h2><div class="syncetc-grid">${textarea("why_join","Why do you want to join?", true)}${textarea("expected_flying","Expected type of flying")}${field("how_hear_us","How did you hear about us?", "text", true)}${field("referred_by","Referred by")}${textarea("availability","Availability for interview/orientation")}</div></section><div id="syncetc-apply-alert"></div><div class="syncetc-actions"><button id="syncetc-apply-submit" class="syncetc-btn" type="submit">Submit Application</button></div>${DEBUG ? `<pre class="syncetc-debug">Apply page ${VERSION}\nOrganization: ${esc(org)}\n${esc(JSON.stringify(state.backend || state.payload || {}, null, 2))}</pre>` : ""}</form></section></div>`;
  }
  ...
  function bind() { const form = document.getElementById("syncetc-apply-form"); if (!form) return; form.addEventListener("submit", async (event) => { event.preventDefault(); if (state.submitting) return; state.submitting = true; const btn = document.getElementById("syncetc-apply-submit"); if (btn) { btn.disabled = true; btn.textContent = "Submitting..."; } alert(""); try { const ds = rootData(); const result = await callPublic("submit_applicant_application", { organization_key: ds.organizationKey || ds.customerKey || "test-customer-1", site_key: ds.siteKey || "primary", page_key: ds.pageKey || "apply-now", ...collect() }); state.submitted = true; mountBody(successHtml(result), state.payload); } catch (error) { alert(error instanceof Error ? error.message : String(error)); } finally { state.submitting = false; if (btn && !state.submitted) { btn.disabled = false; btn.textContent = "Submit Application"; } } }); }

  async function init() { const r = root(); if (!r) return; mark("boot:start", location.pathname); r.innerHTML = `<div style="padding:20px">Loading application…</div>`; markRootReadyFallback(r); try { const ds = rootData(); const payload = await callPublic("get_apply_page", { organization_key: ds.organizationKey || ds.customerKey || "test-customer-1", site_key: ds.siteKey || "primary", page_key: ds.pageKey || "apply-now", render_mode: DEBUG ? "debug" : "public" }); state.payload = payload; mountBody(formHtml(), payload); bind(); } catch (error) { if (window.SyncEtcPublicShell && typeof window.SyncEtcPublicShell.renderError === "function" && state.payload) {
