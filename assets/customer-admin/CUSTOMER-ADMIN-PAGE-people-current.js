...
(function () {
  ...
  function assetCheckoutStatusOptions0116S(value = "inactive") {
    const current = key(value || "inactive") || "inactive";
    return [["inactive", "Not checked out"], ["active", "Approved"], ["pending", "Pending"], ["expired", "Expired"], ["revoked", "Revoked"], ["waived", "Waived"]].map(([v,label]) => `<option value="${esc(v)}" ${current === v ? "selected" : ""}>${esc(label)}</option>`).join("");
  }
  ...
  function renderAircraftCheckoutRows0116S(row, mayEdit) {
    ...
    const rows = aircraft.map((asset) => {
      ...
      const status = key(assignment.checkout_status || assignment.assignment_status || assignment.status || "inactive") || "inactive";
      ...
      return `<div class="people-aircraft-checkout-row" role="row" data-aircraft-checkout-field="${esc(domId)}" data-operational-asset-id="${esc(assetId)}" data-assignment-id="${esc(assignmentId)}">
        <div class="people-aircraft-checkout-title" role="cell"><strong>${esc(assetLabel0116S(asset))}</strong>${meta ? `<small>${esc(meta)}</small>` : ""}</div>
        <div role="cell"><div class="people-qualification-cell-input"><label class="people-field people-qualification-grid-field"><span>Approval</span><select id="people-aircraft-checkout-status-${esc(domId)}" ${disabled}>${assetCheckoutStatusOptions0116S(status)}</select></label></div></div>
        ...
        <div role="cell"><div class="people-qualification-cell-input">${qualificationDateInput0116R(`people-aircraft-checkout-expires-${domId}`, "Current through", expires, disabled)}</div></div>
    ...
    return `<section class="people-qualification-panel people-aircraft-checkout-panel"><div class="people-role-section-head"><h4>Aircraft checkouts</h4></div><div class="people-aircraft-checkout-table-wrap" tabindex="0" aria-label="Aircraft checkouts"><div class="people-aircraft-checkout-table" role="table"><div class="people-aircraft-checkout-row people-qualification-table-head" role="row"><div role="columnheader">Aircraft</div><div role="columnheader">Approval</div><div role="columnheader">Completed</div><div role="columnheader">Current through</div><div role="columnheader">Notes</div></div>${rows}</div></div></section>`;
  }
  ...
  function normalizeExistingAssetCheckoutForSave0116S(assignment = {}) {
    ...
    return {
      ...
      checkout_status: key(a.checkout_status || a.assignment_status || a.status || "active") || "active",
      assignment_status: key(a.checkout_status || a.assignment_status || a.status || "active") || "active",
      status: key(a.checkout_status || a.assignment_status || a.status || "active") || "active",
      ...
      expires_at: clean(a.expires_at || a.expiration_date || a.current_through || "").slice(0,10),
      expiration_date: clean(a.expiration_date || a.expires_at || a.current_through || "").slice(0,10),
      notes: clean(a.notes || ""),
