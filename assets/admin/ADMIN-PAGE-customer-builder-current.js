...
(function () {
    ...
    root.innerHTML = `
      ...
      <main class="se-wrap">
        <section class="se-card">
          ...
          <p class="se-subtitle">Create and manage customer records. This is the first business object in the clean platform rebuild.</p>
        ...
        <section class="se-grid-two">
          <div class="se-card">
            ...
            <label class="se-field">
              <span class="se-label">Customer Key</span>
              <input id="se-create-customer-key" class="se-input" type="text" placeholder="example-flying-club">
            </label>
  ...
  function renderSelectedCustomer() {
    ...
    wrap.innerHTML = `
      ...
      <div class="se-customer-meta">
        ...
        Customer key cannot be edited in v1: ${escapeHtml(customer.customer_key)}
      </div>
  ...
  function clearCreateForm() {
    ["se-create-display-name", "se-create-customer-key", "se-create-legal-name", "se-create-notes"].forEach((id) => {
  ...
  async function createCustomer() {
    ...
    const customerKeyInput = document.getElementById("se-create-customer-key")?.value || "";
    const customerKey = normalizeKey(customerKeyInput || displayName);
    ...
    if (!customerKey) {
      setStatus("Customer Key is required or must be derivable from Display Name.");
      return;
    }
  ...
  function bindEvents() {
    ...
    document.getElementById("se-create-display-name")?.addEventListener("input", () => {
      const keyInput = document.getElementById("se-create-customer-key");
      const displayName = document.getElementById("se-create-display-name")?.value || "";
      if (keyInput && !keyInput.dataset.userEdited) {
        keyInput.value = normalizeKey(displayName);
      }
    });

    document.getElementById("se-create-customer-key")?.addEventListener("input", (event) => {
      event.target.dataset.userEdited = "true";
      event.target.value = normalizeKey(event.target.value);
    });
