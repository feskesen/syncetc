...
(function () {
  ...
  function setShellState() { const row = selectedRow(); window.SyncEtcPortalShell?.setState?.({ authenticated: Boolean(token), email, mode: "org-admin", organizationName: row?.organization_name || "", organizationKey: row?.organization_key || "", styleProfile: row?.style_profile || null, accessRow: row || null }); }
  ...
  async function loadAccess() { const res = await call("get_my_access"); access = res.access || []; if (!selectedOrgId && adminRows()[0]) selectedOrgId = String(adminRows()[0].organization_id); if (selectedOrgId) await loadAdminDashboard(); setShellState(); }
  ...
  function render() {
    ...
    root.innerHTML = `
      ...
      <div class="orgadm-wrap"><section class="orgadm-card orgadm-hero"><div class="orgadm-eyebrow">Organization Admin</div><h1>Organization Admin Dashboard</h1><p>Customer-side administration using the same login as the User Dashboard.</p>${renderLogin()}<div class="orgadm-message ${esc(messageKind)}">${esc(message)}</div></section>${renderDashboard()}<details class="orgadm-card"><summary>Backend result</summary><pre class="orgadm-backend">${esc(JSON.stringify(backend || {}, null, 2))}</pre></details></div>`;
