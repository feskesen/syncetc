...
(function () {
  ...
  function eventImageUrl(event) {
    const asset = obj(event.event_image_asset_json);
    return clean(event.event_image_url || event.image_url || asset.publicUrl || asset.public_url || asset.url || "");
  }
  ...
  function renderNextEvent(event) {
    ...
    const accent = clean(event.event_accent_color || obj(event.event_type_json).accent_color || obj(event.event_type_json).color || "");
    const accentStyle = accent ? `style="--event-accent:${esc(accent)}"` : "";
    return `<section class="md-card md-next-event"><span class="md-eyebrow">Calendar</span><h2>Next club event</h2><div class="md-event-box" ${accentStyle}><div class="md-event-content">${img ? `<div class="md-event-image"><img src="${esc(img)}" alt=""></div>` : `<div class="md-event-image placeholder">📅</div>`}<div class="md-event-main"><div class="md-event-chips"><span class="md-mini">${esc(event.event_type_label || event.category || "Club event")}</span>${event.rsvp_enabled ? `<span class="md-mini ok">RSVP open</span>` : ""}</div><h3>${esc(event.title || "Upcoming event")}</h3>${when ? `<p><strong>${esc(when)}</strong></p>` : ""}${location ? `<p>${esc(location)}</p>` : ""}<a class="md-small-link" href="/calendar">Open full calendar</a></div></div></div></section>`;
  }
  ...
  function renderWeatherCard(w) {
    if (!w?.ok) {
      return `<article class="md-weather-card failed"><div class="md-weather-head"><div><span class="md-eyebrow danger">Weather unavailable</span><h3>${esc(w?.station || "Station")}</h3><p>${esc(w?.label || "")}</p></div><span class="md-category unknown">Error</span></div><p class="md-weather-warning">Unable to retrieve a current METAR for this airport.</p>${DEBUG ? `<pre>${esc(w?.error || "No diagnostic returned.")}</pre>${w?.endpoint ? `<p class="md-help">Endpoint: ${esc(w.endpoint)}</p>` : ""}` : ""}</article>`;
    }
    const details = obj(w.details);
    const raw = clean(w.raw_text || w.raw_metar || "No raw METAR returned.");
    const parts = [details.wind_text, details.visibility_text, details.ceiling_text, details.temperature_text, details.altimeter_text, details.remarks_text].map(clean).filter(Boolean);
    const warning = clean(w.warning || "");
    return `<article class="md-weather-card ${warning ? "is-stale" : ""}"><div class="md-weather-head"><div><span class="md-eyebrow">${esc(w.station)}</span><h3>${esc(w.label || w.station)}</h3></div><span class="md-category ${weatherClass(w.flight_category)}">${esc(w.flight_category || "Unknown")}</span></div>${warning ? `<p class="md-weather-warning">${esc(warning)}</p>` : ""}<div class="md-raw">${esc(raw)}</div>${parts.length ? `<div class="md-weather-details">${parts.map((p) => `<span>${esc(p)}</span>`).join("")}</div>` : ""}<p class="md-help"><strong>Observed:</strong> ${esc(w.observed_utc || "unknown UTC")}${w.observed_local ? ` · ${esc(w.observed_local)} local` : ""}</p><p class="md-help"><strong>Last updated:</strong> ${esc(w.fetched_utc || "unknown UTC")}${w.fetched_local ? ` · ${esc(w.fetched_local)} local` : ""}</p>${DEBUG && w.last_error_message ? `<p class="md-help"><strong>Last refresh error:</strong> ${esc(w.last_error_message)}</p>` : ""}</article>`;
  }

  function renderWeather() {
    const weather = arr(dashboard?.weather);
    return `<section class="md-card md-weather-section"><div class="md-section-head"><div><span class="md-eyebrow">Airport weather</span><h2>METARs</h2></div><span class="md-mini">AviationWeather.gov</span></div><div class="md-weather-grid">${weather.length ? weather.map(renderWeatherCard).join("") : `<article class="md-weather-card failed"><h3>Weather not loaded</h3><p>No weather payload was returned by the backend.</p></article>`}</div><p class="md-disclaimer">Weather Disclaimer: METAR and flight category data are provided as a member convenience only. This is not a substitute for an official weather briefing. Always confirm conditions through official channels such as Flight Service (1-800-WX-BRIEF), the FAA Weather Briefing Portal, or another FAA-approved source before flight.</p></section>`;
  }
