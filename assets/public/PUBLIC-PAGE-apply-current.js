...
(function () {
  ...
  function successHtml(result) { const portal = result.applicant_portal || {}; const portalBlock = portal.available ? `<div class="syncetc-alert ok"><strong>Applicant portal:</strong> If your organization allows applicant portal access at this stage, you can request a secure login link at <a href="${esc(portal.portal_url || '/applicant-portal')}">Applicant Portal</a>.</div>` : `<div class="syncetc-alert">If the organization requests more information, you will receive instructions by email.</div>`; return `<div class="syncetc-apply"><section class="syncetc-apply-card"><div class="syncetc-apply-hero"><div class="syncetc-apply-eyebrow">Application Received</div><h1>Thank you for applying.</h1><p>Your application has been received. The organization will review it and follow up if more information is needed.</p></div><div class="syncetc-apply-body"><div class="syncetc-alert ok">Application received. Reference: ${esc(result.applicant_key || result.application_id || "received")}</div>${portalBlock}${DEBUG ? `<pre class="syncetc-debug">Apply page ${VERSION}
