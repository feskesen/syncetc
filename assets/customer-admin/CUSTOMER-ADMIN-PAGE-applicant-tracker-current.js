...
// Purpose: Organization admin Applicant Tracker with archive reason workflow, settings modal, notes/activity timeline, member conversion, local filters, and lifecycle-ready tracker UX.
...
(function () {
  ...
  function statusOptions(selected, includeAll=false){ return STATUS.map(([k,l])=>`<option value="${esc(k)}" ${selected===k?'selected':''}>${esc(l)}</option>`).join(""); }
...
${esc(JSON.stringify(app,null,2))}</pre>`:''}</main>`;
  ...
  function convertModal(){
    ...
    const matches=lookups.matches;
    const selectedMatch=clean(state.conversionPersonId);
    const canSubmit = state.conversionMode==='create_new' || selectedMatch;
    return `<div class="at-modal-backdrop" id="at-convert-modal-backdrop"><div class="at-modal" role="dialog" aria-modal="true"><div class="at-modal-head"><div><h2>Add Applicant as Member</h2><p style="margin:4px 0 0;color:rgba(255,255,255,.9);font-weight:800">This closes the applicant lifecycle and links the applicant to a People/member record.</p></div><button class="at-close" id="at-close-convert" type="button">×</button></div><div class="at-modal-body"><div class="at-alert info">Applicant records are preserved for history. Conversion archives the applicant with reason <strong>Added as Member</strong>.</div>${state.conversionLoading?'<div class="at-card">Loading conversion options…</div>':`<div class="at-grid"><label class="at-field"><span class="at-label">Conversion choice</span><select id="at-convert-mode" class="at-select"><option value="create_new" ${state.conversionMode==='create_new'?'selected':''}>Create new person</option><option value="link_existing" ${state.conversionMode==='link_existing'?'selected':''}>Link existing person</option></select></label><label class="at-field"><span class="at-label">Applicant</span><input class="at-input" value="${attr(app.display_name||'Applicant')}" readonly></label></div>${state.conversionMode==='link_existing'?`<div><h3 style="margin:8px 0;color:var(--at-primary,#265c2b)">Possible people matches</h3>${matches.length?matches.map(m=>`<label class="at-match ${selectedMatch===clean(m.person_id)?'selected':''}"><input type="radio" name="at-convert-person" value="${attr(m.person_id)}" ${selectedMatch===clean(m.person_id)?'checked':''}> <strong>${esc(m.display_name||'Person')}</strong><div class="at-meta">${esc(m.primary_email||'')} ${m.primary_phone?`• ${esc(m.primary_phone)}`:''} • ${esc(m.match_reason||'Possible match')}${m.has_active_membership?' • active membership exists':''}</div></label>`).join(''):'<div class="at-empty">No likely existing people were found. Choose create new person.</div>'}</div>`:''}<div class="at-grid"><label class="at-field"><span class="at-label">Initial lifecycle status</span><select id="at-convert-status" class="at-select">${membershipStatusOptions(lookups.statuses, state.conversionStatusKey)}</select></label><label class="at-field"><span class="at-label">Initial member class</span><select id="at-convert-class" class="at-select">${membershipClassOptions(lookups.classes, state.conversionClassKey)}</select></label><label class="at-field"><span class="at-label">Initial onboarding/application stage</span><select id="at-convert-stage" class="at-select">${applicationStageOptions(lookups.stages, state.conversionStageKey)}</select></label><label class="at-field"><span class="at-label">Conversion note</span><input id="at-convert-note" class="at-input" value="${attr(state.conversionNote)}" placeholder="Optional note for the timeline"></label></div><div class="at-alert info">Flight scheduler access is seeded as pending checkout. The scheduler itself is not enabled in this pass.</div><div class="at-actions"><button class="at-btn" id="at-confirm-convert" ${!canSubmit||state.conversionSaving?'disabled':''}>${state.conversionSaving?'Converting…':'Confirm conversion'}</button><button class="at-btn secondary" id="at-cancel-convert">Cancel</button></div>`}</div></div></div>`;
  }
  ...
  async function openConvertModal(){
    ...
    try{
      ...
      if(arr(res.matches).length) { state.conversionMode='link_existing'; state.conversionPersonId=clean(res.matches[0].person_id); }
  ...
  async function confirmConvertApplicant(){
    if(!state.selected) return;
    const payload={ action:'organization_convert_applicant_to_member', organization_id:state.orgId, application_id:state.selected.application_id, conversion_mode:state.conversionMode, person_id:state.conversionPersonId, membership_status_key:val('at-convert-status')||state.conversionStatusKey, membership_class_key:val('at-convert-class')||state.conversionClassKey, application_stage_key:val('at-convert-stage')||state.conversionStageKey, conversion_note:val('at-convert-note')||state.conversionNote };
    if(!payload.membership_status_key && !payload.membership_class_key && !payload.application_stage_key){ state.message='Choose at least one initial member status, class, or stage before conversion.'; state.messageKind='bad'; render(); return; }
