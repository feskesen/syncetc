...
(function () {
  ...
  let options = { statuses: [], membership_classes: [], application_stages: [], roles: [], permissions: [], qualifications: [], assets: [], aircraft: [], asset_qualification_requirements: [] };
  ...
  function confirmDiscard(message = DIRTY_MESSAGE) {
    ...
    const ok = confirm(message || DIRTY_MESSAGE);
  ...
  async function refreshBearerTokenForRetry0116Q() {
    try {
      ...
      if (refreshedSession?.access_token) {
        ...
        email = refreshedSession?.user?.email || email;
        return true;
      }
    } catch {}
    try {
  ...
  function isJwtFailure0116Q(res, json = {}) {
    const msg = lower(json.message || json.error || json.msg || "");
    return res?.status === 401 || msg.includes("invalid jwt") || msg.includes("jwt expired") || msg.includes("jwt");
  }
  ...
  async function call(action, payload = {}) {
    ...
    for (;;) {
      ...
      if ((res.ok && json.ok !== false) || !isJwtFailure0116Q(res, json) || attemptedRefresh) {
        if (!res.ok || json.ok === false) throw new Error(json.message || json.error || `Action failed: ${action}`);
        return json;
      }
      attemptedRefresh = true;
      markLoad("auth retry", "refreshing session after JWT failure");
      const refreshed = await refreshBearerTokenForRetry0116Q();
      if (!refreshed) throw new Error(json.message || json.error || `Action failed: ${action}`);
    }
  }
  ...
  async function logout() { if (!confirmDiscard()) return; setDirty(false); await ensureSupabase(); await supabaseClient.auth.signOut(); token = ""; email = ""; allAccess = []; adminAccess = null; selectedOrgId = ""; people = []; availableAircraft = []; selected = null; roleFilter = "all"; loginFilter = "all"; qualificationFilter = "all"; options = { statuses: [], membership_classes: [], application_stages: [], roles: [], permissions: [], qualifications: [], assets: [], aircraft: [], asset_qualification_requirements: [] }; definitionLists = { statuses: [], membership_classes: [], application_stages: [], groups_roles: [], permissions: [], qualifications: [] }; selectedDefinition = null; selectedPermissionRoleId = ""; permissionDraftKeys = null; authChecked = true; setShellState(); render(); }
  ...
  function applyPeopleListResult(res) {
    ...
    people = res?.people || [];
    const aircraft = arr(res?.checkout_assets || res?.available_aircraft || res?.aircraft_options || res?.assets || res?.aircraft || res?.operational_assets);
    if (aircraft.length) {
      availableAircraft = aircraft;
      options = { ...options, assets: aircraft, aircraft, asset_qualification_requirements: options.asset_qualification_requirements || [] };
    }
  ...
  async function loadOrgContext(loadOptions = {}) {
    ...
    if (!loadOptions.force) {
      ...
      if (cached) {
        ...
        options = cached.options || { statuses: [], membership_classes: [], application_stages: [], roles: [], permissions: [], qualifications: [], assets: [], aircraft: [], asset_qualification_requirements: [] };
        availableAircraft = arr(cached.availableAircraft || cached.options?.aircraft || cached.options?.assets);
    ...
    options = { statuses: vocab.statuses || [], membership_classes: vocab.membership_classes || [], application_stages: vocab.application_stages || [], roles: sortRoles(vocab.roles || []), permissions: vocab.permissions || [], qualifications: vocab.qualifications || [], assets: vocab.assets || [], aircraft: vocab.aircraft || vocab.assets || [], asset_qualification_requirements: vocab.asset_qualification_requirements || [] };
    if (arr(options.aircraft || options.assets).length && !arr(availableAircraft).length) availableAircraft = arr(options.aircraft || options.assets);
  ...
  function normalizeDefinitionRow(row, type) {
    ...
    if (type === "lifecycle-statuses") {
      ...
      r.required_to_reserve = r.required_to_reserve === true || settings.required_to_reserve === true;
      r.ui_status = definitionUiStatus(r);
      r.sort_order = Number(r.sort_order || 100);
    } else {
      const settings = obj(r.settings_json);
  ...
  function blankDefinition(kind = activeDefinitionKind) {
    ...
    if (cfg.kind === "qualifications") {
      return { qualification_definition_id: "", qualification_key: "", label: "", description: "", qualification_type: "checkout", applies_to: "member", requires_expiration_date: true, requires_document: false, requires_approval: false, is_default: false, status: "active", ui_status: "active", sort_order: nextDefinitionSortOrder(), settings_json: { field_style: "checkbox_expiration", show_on_profile: true, required_to_reserve: false, option_values: [] } };
    }
  ...
  function instructorQualificationKeys0116R(row = {}) {
    const qk = qualificationDefinitionKey(row);
    return ["cfi", "cfii", "mei"].includes(qk);
  }

  function defaultQualificationIssuedLabel0116R(row = {}, style = "") {
    const qk = qualificationDefinitionKey(row);
    if (style === "date_expiration") return "Completed";
    if (qk === "medical-certificate") return "Last medical date";
    if (instructorQualificationKeys0116R(row)) return "Credential issued";
    return "Issued";
  }

  function defaultQualificationExpirationLabel0116R(row = {}) {
    const qk = qualificationDefinitionKey(row);
    if (instructorQualificationKeys0116R(row)) return "Privileges current through";
  ...
  function qualificationSettings0116O(row = {}) {
    ...
    return {
      ...
      issued_label: rawIssuedLabel || defaultQualificationIssuedLabel0116R(row, normalizedStyle),
      expiration_label: rawExpirationLabel && !(instructorQualificationKeys0116R(row) && lower(rawExpirationLabel) === "certificate expires") ? rawExpirationLabel : defaultQualificationExpirationLabel0116R(row),
      notes_label: clean(settings.notes_label || "Notes"),
      show_on_profile: settings.show_on_profile !== false,
      required_to_reserve: row.required_to_reserve === true || settings.required_to_reserve === true
    };
  }
  ...
  function qualificationRequiredReserveCheckbox0116R(checked = false, disabled = false) {
    const tip = "If required, this qualification is included in the person\'s reservation readiness check. Asset-specific rules can be added later.";
    return `<label class="people-check ${disabled ? "disabled" : ""}"><input id="people-def-required-reserve" type="checkbox" ${checked ? "checked" : ""} ${disabled ? "disabled" : ""}><span>Required to reserve <button class="people-info-btn" type="button" tabindex="0" aria-label="${esc(tip)}" title="${esc(tip)}">?</button></span></label>`;
  }
  ...
  function renderDefinitionRow(row) {
    ...
    return `<div class="people-def-row people-person-card ${selectedClass} ${muted}" data-def-row data-def-id="${esc(id)}" draggable="${canMove ? "true" : "false"}">
      ...
      <button class="people-def-main" data-def-open="${esc(id)}" type="button"><span class="person-main"><strong>${esc(row.label || "Untitled")}</strong><small>${esc(meta || "No notes")}</small></span><span class="person-badges">${status === "archived" ? "<em>Archived</em>" : status === "inactive" ? "<em>Inactive</em>" : ""}${row.is_default ? "<em>Default</em>" : ""}${cfg.kind === "qualifications" && qualificationSettings0116O(row).required_to_reserve ? "<em>Required</em>" : ""}</span></button>
  ...
  function collectDefinitionPayload() {
    ...
    if (cfg.kind === "membership-classes") {
      ...
      base.required_to_reserve = bool($("people-def-required-reserve")?.checked);
      base.settings_json = {
        ...
        show_on_profile: bool($("people-def-show-profile")?.checked) || bool($("people-def-required-reserve")?.checked),
        required_to_reserve: bool($("people-def-required-reserve")?.checked)
      };
    } else if (cfg.kind === "groups-roles") {
  ...
  function checkboxRich(id, labelHtml, checked = false, disabled = false) {
    return `<label class="people-check ${disabled ? "disabled" : ""}"><input id="${esc(id)}" type="checkbox" ${checked ? "checked" : ""} ${disabled ? "disabled" : ""}><span>${labelHtml}</span></label>`;
  }
  function checkboxInfo(id, label, tip, checked = false, disabled = false) {
    const info = tip ? ` <button class="people-info-btn" type="button" tabindex="0" aria-label="${esc(tip)}" title="${esc(tip)}">?</button>` : "";
    return `<label class="people-check ${disabled ? "disabled" : ""}"><input id="${esc(id)}" type="checkbox" ${checked ? "checked" : ""} ${disabled ? "disabled" : ""}><span>${esc(label)}${info}</span></label>`;
  }
  ...
  function visibleQualificationDefinitions0116R() {
    return sortedQualificationDefinitions().filter((definition) => {
      const status = definitionUiStatus(definition);
      const settings = qualificationSettings0116O(definition);
      return status === "active" && (settings.show_on_profile !== false || settings.required_to_reserve === true);
    });
  }

  function qualificationCellInput0116R(html) {
    return `<div class="people-qualification-cell-input">${html || `<span class="people-qualification-empty">—</span>`}</div>`;
  }

  function qualificationDateInput0116R(id, label, value, disabled) {
  ...
  function todayIso0116R() { return new Date().toISOString().slice(0,10); }

  function qualificationExpiresMissingOrPast0116R(definition, assignment) {
    const settings = qualificationSettings0116O(definition);
    const style = key(settings.field_style || "checkbox");
    const needsDate = bool(definition.requires_expiration_date || ["checkbox_expiration", "date_expiration", "class_expiration", "status_expiration"].includes(style));
    if (!needsDate) return false;
    const expires = clean(assignment?.expiration_date || assignment?.expires_at || "").slice(0,10);
    if (!expires) return true;
    return expires < todayIso0116R();
  }

  function qualificationRequirementStatus0116R(definition, row = selected || {}) {
    const assignment = qualificationAssignmentFor(definition, row) || {};
    const settings = qualificationSettings0116O(definition);
    const style = key(settings.field_style || "checkbox");
    const assignmentSettings = obj(assignment.settings_json);
    const checked = isQualificationChecked(definition, row);
    const status = key(assignment.assignment_status || assignment.status || "");
    ...
    const expiresBad = qualificationExpiresMissingOrPast0116R(definition, assignment);
    let hasValue = false;
    if (["checkbox", "checkbox_expiration"].includes(style)) hasValue = checked;
    else if (style === "class_expiration") hasValue = Boolean(optionValue && key(optionValue) !== "none");
    else if (style === "date_expiration") hasValue = Boolean(issued || clean(assignment.expiration_date || assignment.expires_at));
    else if (style === "status_expiration") hasValue = ["active", "waived"].includes(status);
    else if (style === "notes") hasValue = Boolean(clean(assignment.notes));
    else hasValue = checked || Boolean(optionValue || issued || clean(assignment.notes));
    if (!hasValue) return { ok: false, reason: "missing" };
    if (expiresBad) return { ok: false, reason: "expired" };
    return { ok: true, reason: "ok" };
  }

  function reservationReadiness0116R(row = selected || {}) {
    const required = visibleQualificationDefinitions0116R().filter((definition) => qualificationSettings0116O(definition).required_to_reserve);
    const missing = [];
    const expired = [];
    required.forEach((definition) => {
      const status = qualificationRequirementStatus0116R(definition, row);
      if (status.ok) return;
      const label = qualificationLabel(definition);
      if (status.reason === "expired") expired.push(label);
      else missing.push(label);
    });
    return { required, missing, expired };
  }

  function renderReservationReadiness0116R(row = selected || {}) {
    return "";
  }

  function assetQualificationRequirementRows0117C(assetId = "") {
    const id = clean(assetId);
    if (!id) return [];
    return arr(options.asset_qualification_requirements || options.assetQualificationRequirements || []).map(obj).filter((requirement) => {
      const requirementAssetId = clean(requirement.operational_asset_id || requirement.asset_id || requirement.aircraft_id);
      if (requirementAssetId !== id) return false;
      if (requirement.archived_at) return false;
      return requirement.is_required !== false && requirement.required !== false;
    });
  }

  function qualificationDefinitionById0117C(definitionId = "") {
    const id = clean(definitionId);
    if (!id) return null;
    return sortedQualificationDefinitions().find((definition) => qualificationDefinitionId(definition) === id) || null;
  }

  function assetRequiresSpecificCheckout0117C(asset = {}) {
    const settings = obj(asset.settings_json);
    const operational = obj(asset.operational_json);
    if (settings.asset_specific_checkout_required === false || operational.asset_specific_checkout_required === false || asset.asset_specific_checkout_required === false) return false;
    return true;
  }

  function qualificationRequirementIssueLabel0117C(definition, row = selected || {}) {
    const status = qualificationRequirementStatus0116R(definition, row);
    if (status.ok) return "";
    const label = qualificationLabel(definition);
    return status.reason === "expired" ? `${label} expired` : `Missing ${label}`;
  }

  function aircraftReservationRequirements0117C(asset = {}) {
    const assetId = clean(asset.operational_asset_id || asset.asset_id || asset.aircraft_id);
    const byId = new Map();
    visibleQualificationDefinitions0116R().filter((definition) => qualificationSettings0116O(definition).required_to_reserve).forEach((definition) => {
      const id = qualificationDefinitionId(definition);
      if (id) byId.set(id, definition);
    });
    assetQualificationRequirementRows0117C(assetId).forEach((requirement) => {
      const definition = qualificationDefinitionById0117C(requirement.qualification_definition_id || requirement.definition_id || requirement.qualification_id);
  ...
  function aircraftReservationEligibility0117C(asset = {}, row = selected || {}, assignment = {}) {
    const issues = [];
    const checkoutRequired = assetRequiresSpecificCheckout0117C(asset);
    ...
    if (checkoutRequired) {
      if (!authorized) issues.push("Aircraft checkout missing");
      else if (expires && expires < todayIso0116R()) issues.push("Aircraft checkout expired");
    }
    aircraftReservationRequirements0117C(asset).forEach((definition) => {
      const issue = qualificationRequirementIssueLabel0117C(definition, row);
      if (issue) issues.push(issue);
    });
    return { ready: !issues.length, issues, requirement_count: aircraftReservationRequirements0117C(asset).length, checkout_required: checkoutRequired };
  }

  function aircraftEligibilitySummary0117C(eligibility) {
    const issues = arr(eligibility.issues).filter(Boolean);
    if (!issues.length) return "All reservation requirements look current.";
    return issues.slice(0, 4).join(" · ") + (issues.length > 4 ? ` · +${issues.length - 4} more` : "");
  }

  function renderQualificationAssignmentRows(row, mayEdit) {
    const definitions = visibleQualificationDefinitions0116R();
    ...
    return `<section class="people-qualification-panel people-qualification-grid-panel"><div class="people-role-section-head"><h4>Qualifications</h4></div><div class="people-qualification-table-wrap" tabindex="0" aria-label="Qualification fields"><div class="people-qualification-table" role="table"><div class="people-qualification-table-row people-qualification-table-head" role="row"><div role="columnheader">Qualification</div><div role="columnheader">Status / value</div><div role="columnheader">Completed / issued</div><div role="columnheader">Expires / current through</div><div role="columnheader">Notes</div></div>${definitions.map((definition) => {
      const id = qualificationDefinitionId(definition);
      ...
      const qKey = qualificationDefinitionKey(definition);
      const qType = key(definition.qualification_type || obj(definition.settings_json).qualification_type);
      const checkLabel = fieldStyle === "checkbox" ? "Yes" : (["cfi","cfii","mei"].includes(qKey) ? "Privileges current" : (["club-instructor","safety-pilot-approved"].includes(qKey) ? "Approved" : (qType === "checkout" || qKey.includes("checkout") ? "Approved" : "Current")));
      ...
      if (fieldStyle === "class_expiration") {
        ...
        issuedCell = qualificationDateInput0116R(`people-qual-issued-${domId}`, settings.issued_label || "Issued", issued, disabled);
        expiresCell = qualificationDateInput0116R(`people-qual-expires-${domId}`, settings.expiration_label || "Expires", expires, disabled);
      } else if (fieldStyle === "date_expiration") {
        issuedCell = qualificationDateInput0116R(`people-qual-issued-${domId}`, settings.issued_label || "Completed", issued, disabled);
        expiresCell = qualificationDateInput0116R(`people-qual-expires-${domId}`, settings.expiration_label || "Expires", expires, disabled);
      ...
      const reserveStatus = qualificationRequirementStatus0116R(definition, row);
      const reserveWarning = settings.required_to_reserve && !reserveStatus.ok ? (reserveStatus.reason === "expired" ? "Required to reserve — expired" : "Required to reserve") : "";
      return `<div class="people-qualification-table-row ${reserveWarning ? "needs-attention" : ""}" role="row" data-qualification-field="${esc(domId)}" data-qualification-definition-id="${esc(id)}" data-qualification-key="${esc(qualificationDefinitionKey(definition))}" data-field-style="${esc(fieldStyle)}" data-assignment-id="${esc(clean(assignment.person_qualification_assignment_id || assignment.qualification_assignment_id || ""))}">
        <div class="people-qualification-title" role="cell"><strong>${esc(qualificationLabel(definition))}</strong>${definition.description ? `<small>${esc(definition.description)}</small>` : ""}${reserveWarning ? `<em class="alert">${esc(reserveWarning)}</em>` : ""}</div>
        <div role="cell">${qualificationCellInput0116R(valueCell)}</div>
        <div role="cell">${qualificationCellInput0116R(issuedCell)}</div>
        <div role="cell">${qualificationCellInput0116R(expiresCell)}</div>
        <div role="cell">${qualificationCellInput0116R(notesCell)}</div>
      </div>`;
    }).join("")}</div></div></section>`;
  }

  function assetLabel0116S(asset = {}) {
    const tail = clean(asset.tail_number || asset.registration || asset.n_number);
    const name = clean(asset.display_name || asset.asset_name || asset.name || asset.label || asset.preferred_name || asset.public_label);
    if (tail && name && !lower(name).includes(lower(tail))) return `${tail} — ${name}`;
    return name || tail || clean(asset.asset_key || asset.operational_asset_id || "Aircraft");
  }

  function availableAircraftForCheckouts0116S() {
    const source = arr(availableAircraft).length ? arr(availableAircraft) : arr(options.aircraft || options.assets || options.operational_assets);
    return source.filter((asset) => {
      const id = clean(asset.operational_asset_id || asset.asset_id || asset.aircraft_id);
      if (!id || asset.archived_at) return false;
      const status = key(asset.status || asset.record_status || asset.asset_record_status || "active");
      return !["archived", "inactive"].includes(status);
    }).sort((a, b) => assetLabel0116S(a).localeCompare(assetLabel0116S(b)));
  }

  function assetCheckoutRowsFor0116S(row = selected || {}) {
    return arr(row.asset_checkout_assignments || row.aircraft_checkouts || row.person_asset_checkouts).map(obj).filter((assignment) => !assignment.archived_at);
  }

  function assignmentAssetId0116S(assignment = {}) {
    return clean(assignment.operational_asset_id || assignment.asset_id || assignment.aircraft_id);
  }
  ...
  function assetCheckoutBulkPanel0116T(aircraft, mayEdit) {
    if (!mayEdit || !aircraft.length) return "";
    const itemRows = aircraft.map((asset) => {
      const assetId = clean(asset.operational_asset_id || asset.asset_id || asset.aircraft_id);
      if (!assetId) return "";
  ...
  function renderAircraftCheckoutRows0116S(row, mayEdit) {
    const aircraft = availableAircraftForCheckouts0116S();
    const assignments = assetCheckoutRowsFor0116S(row);
    const byAssetId = new Map(assignments.map((assignment) => [assignmentAssetId0116S(assignment), assignment]).filter(([id]) => Boolean(id)));
    const bulkToggle = mayEdit && aircraft.length ? `<button id="people-aircraft-bulk-toggle" class="people-btn secondary" type="button">Bulk update</button>` : "";
    if (!aircraft.length) {
    ...
    const rows = aircraft.map((asset) => {
      ...
      const checkoutBadge = !authorized ? "Not authorized" : (expired ? "Checkout expired" : "Authorized");
      return `<div class="people-aircraft-checkout-row ${expired ? "needs-attention" : ""}" role="row" data-aircraft-checkout-field="${esc(domId)}" data-operational-asset-id="${esc(assetId)}" data-assignment-id="${esc(assignmentId)}">
        <div class="people-aircraft-checkout-title" role="cell"><strong>${esc(assetLabel0116S(asset))}</strong>${meta ? `<small>${esc(meta)}</small>` : ""}${authorized || expired ? `<em class="${expired ? "alert" : "ready"}">${esc(checkoutBadge)}</em>` : ""}</div>
  ...
  function renderReservationEligibilityPreview0117D(row = selected || {}) {
    const aircraft = availableAircraftForCheckouts0116S();
    if (!aircraft.length) return "";
    const assignments = assetCheckoutRowsFor0116S(row);
    const byAssetId = new Map(assignments.map((assignment) => [assignmentAssetId0116S(assignment), assignment]).filter(([id]) => Boolean(id)));
    const previewRows = aircraft.map((asset) => {
      const assetId = clean(asset.operational_asset_id || asset.asset_id || asset.aircraft_id);
      if (!assetId) return "";
      const assignment = byAssetId.get(assetId) || {};
      const eligibility = aircraftReservationEligibility0117C(asset, row, assignment);
      const requirements = aircraftReservationRequirements0117C(asset);
      if (!eligibility.checkout_required && !requirements.length) return "";
      const summary = aircraftEligibilitySummary0117C(eligibility);
      const meta = [clean(asset.aircraft_make || asset.make), clean(asset.aircraft_model || asset.model)].filter(Boolean).join(" ");
      return `<div class="people-reservation-preview-row ${eligibility.ready ? "ready" : "needs-attention"}" role="row"><div role="cell"><strong>${esc(assetLabel0116S(asset))}</strong>${meta ? `<small>${esc(meta)}</small>` : ""}</div><div role="cell"><span class="people-mini-pill ${eligibility.ready ? "ok" : "warn"}">${esc(eligibility.ready ? "Ready" : "Needs attention")}</span></div><div role="cell"><small>${esc(summary)}</small></div></div>`;
    }).filter(Boolean).join("");
    if (!previewRows) return "";
    return `<section class="people-qualification-panel people-reservation-preview-panel"><div class="people-role-section-head"><div><h4>Reservation eligibility preview</h4><small>Separate from aircraft checkouts; use this only to review reservation readiness.</small></div></div><div class="people-reservation-preview-table" role="table"><div class="people-reservation-preview-row people-qualification-table-head" role="row"><div role="columnheader">Aircraft</div><div role="columnheader">Result</div><div role="columnheader">Needs attention</div></div>${previewRows}</div></section>`;
  }
  ...
  function normalizeExistingAssetCheckoutForSave0116S(assignment = {}) {
    ...
    return {
      ...
      person_asset_checkout_assignment_id: clean(a.person_asset_checkout_id || a.person_asset_checkout_assignment_id || a.asset_checkout_assignment_id || a.checkout_assignment_id),
      operational_asset_id: assignmentAssetId0116S(a),
  ...
  function collectAssetCheckoutAssignments0116S() {
    const visibleFields = Array.from(document.querySelectorAll("[data-aircraft-checkout-field]"));
    const renderedAssetIds = new Set();
    const visibleAssignments = visibleFields.map((field) => {
      const domId = clean(field.getAttribute("data-aircraft-checkout-field"));
    ...
    const hiddenExisting = assetCheckoutRowsFor0116S(selected || {}).filter((assignment) => {
      ...
      return assetId && !renderedAssetIds.has(assetId);
    }).map(normalizeExistingAssetCheckoutForSave0116S).filter((assignment) => assignment.operational_asset_id);
  ...
  function bindAircraftBulkEvents0116T() {
    ...
    if (toggle && panel) {
      toggle.addEventListener("click", () => {
        ...
        toggle.textContent = panel.hidden ? "Bulk update" : "Hide bulk update";
      });
    }
    $("people-aircraft-bulk-select-all")?.addEventListener("click", () => {
      document.querySelectorAll("[data-aircraft-bulk-id]").forEach((box) => { box.checked = true; });
      setBulkStatusMessage0116T("");
    });
    $("people-aircraft-bulk-clear")?.addEventListener("click", () => {
  ...
  function normalizeExistingQualificationAssignmentForSave0116R(assignment = {}) {
    const a = obj(assignment);
  ...
  function collectQualificationAssignments() {
    ...
    const hiddenExisting = qualificationAssignmentsFor(selected || {}).filter((assignment) => {
      ...
      return Boolean(id || k);
    }).map(normalizeExistingQualificationAssignmentForSave0116R).filter((assignment) => assignment.qualification_definition_id || assignment.qualification_key);
...
.people-reservation-preview-panel{overflow:hidden}.people-reservation-preview-panel .people-role-section-head p{margin:3px 0 0;color:var(--people-muted);font-size:12px;line-height:1.3;font-weight:750}.people-reservation-preview-table{display:grid;border:1px solid var(--people-border);border-radius:13px;overflow:hidden;background:#fff}.people-reservation-preview-row{display:grid;grid-template-columns:minmax(210px,1.1fr) minmax(140px,.6fr) minmax(280px,1.6fr);align-items:center;border-bottom:1px solid var(--people-border);background:#fff}.people-reservation-preview-row:last-child{border-bottom:0}.people-reservation-preview-row.needs-attention{background:#fff7ed}.people-reservation-preview-row>div{padding:9px 10px;min-width:0}.people-reservation-preview-row small{color:var(--people-muted);font-weight:800;line-height:1.25}.people-reservation-preview-row.needs-attention small{color:#9a3412}.people-reservation-preview-row.head,.people-reservation-preview-row.people-qualification-table-head{background:#f8fafc}.people-reservation-preview-row .people-pill{display:inline-flex;border-radius:999px;padding:5px 8px;font-size:11px;font-weight:950}.people-reservation-preview-row .people-pill.ok{background:#eaf7ef;color:var(--people-primary)}.people-reservation-preview-row .people-pill.warn{background:#fff1c2;color:#9a3412;border:1px solid #fed7aa}@media(max-width:760px){.people-reservation-preview-table{overflow-x:auto}.people-reservation-preview-row{min-width:620px}}@media(max-width:960px){.people-aircraft-bulk-list{grid-template-columns:1fr}.people-aircraft-bulk-controls{grid-template-columns:1fr 1fr}}
  ...
  function bindEvents() {
    ...
    document.querySelectorAll(".people-editor input, .people-editor select, .people-editor textarea").forEach((el) => {
      if (el.matches("[data-people-ui-only]")) return;
  ...
  function readForm() {
    ...
    const assetCheckoutAssignments = collectAssetCheckoutAssignments0116S();
    ...
    payload.asset_checkout_assignments = assetCheckoutAssignments;
    return payload;
  }
