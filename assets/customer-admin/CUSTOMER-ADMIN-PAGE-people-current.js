...
(function () {
  ...
  function initialsFromName(name) {
    return clean(name).split(" ").filter(Boolean).slice(0,2).map((part) => part[0]).join("").toUpperCase() || "?";
  }
  ...
  async function uploadSelectedPhoto(file) {
    ...
    if (!allowed.has(String(file.type || "").toLowerCase())) throw new Error("Photo must be a JPG, PNG, or WebP image.");
    if (file.size > 5 * 1024 * 1024) throw new Error("Photo is too large. Use an image under 5 MB.");
    setMessage("Uploading photo…", "");
    const dataUrl = await readFileAsDataUrl(file);
    const membershipId = selected.membership_id;
    const res = await call("organization_upload_person_photo", { organization_id: selectedOrgId, person_id: selected.person_id, membership_id: membershipId, file_name: file.name, content_type: file.type, data_url: dataUrl });
    await loadPeople();
    selected = people.find((p) => p.membership_id === (res.person?.membership_id || membershipId)) || res.person || selected;
    setDirty(false);
  ...
  async function removeSelectedPhoto() {
    ...
    if (!photoUrl(selected)) throw new Error("This person does not have a photo to remove.");
    if (!confirm("Remove this person's profile photo?")) return;
    const membershipId = selected.membership_id;
    const res = await call("organization_remove_person_photo", { organization_id: selectedOrgId, person_id: selected.person_id, membership_id: membershipId });
    await loadPeople();
    selected = people.find((p) => p.membership_id === (res.person?.membership_id || membershipId)) || res.person || selected;
    setDirty(false);
  ...
  function bindEvents() {
    ...
    $("people-photo-input")?.addEventListener("change", (event) => {
      const file = event.target?.files?.[0];
      if (!file) return;
      runButton("people-photo-choose", "Uploading…", () => uploadSelectedPhoto(file));
    });
    $("people-photo-remove")?.addEventListener("click", () => runButton("people-photo-remove", "Removing…", removeSelectedPhoto));
    const dropZone = $("people-photo-dropzone");
    if (dropZone && dropZone.getAttribute("data-photo-disabled") !== "true") {
      dropZone.addEventListener("dragover", (event) => { event.preventDefault(); dropZone.classList.add("dragover"); });
      dropZone.addEventListener("dragleave", () => dropZone.classList.remove("dragover"));
      dropZone.addEventListener("drop", (event) => { event.preventDefault(); dropZone.classList.remove("dragover"); const file = event.dataTransfer?.files?.[0]; if (file) runButton("people-photo-choose", "Uploading…", () => uploadSelectedPhoto(file)); });
    }
    document.querySelectorAll(".people-editor input, .people-editor select, .people-editor textarea").forEach((el) => {
      if (el.id === "people-photo-input") return;
