...
(function () {
  ...
  function renderAircraftCheckoutRows0116S(row, mayEdit) {
    ...
    const rows = aircraft.map((asset) => {
      ...
      const eligibilityLabel = eligibility.ready ? "Ready to reserve" : "Needs attention";
      const eligibilitySummary = aircraftEligibilitySummary0117C(eligibility);
      return `<div class="people-aircraft-checkout-row ${!eligibility.ready || expired ? "needs-attention" : ""}" role="row" data-aircraft-checkout-field="${esc(domId)}" data-operational-asset-id="${esc(assetId)}" data-assignment-id="${esc(assignmentId)}">
        <div class="people-aircraft-checkout-title" role="cell"><strong>${esc(assetLabel0116S(asset))}</strong>${meta ? `<small>${esc(meta)}</small>` : ""}<em class="${eligibility.ready ? "ready" : "alert"}">${esc(eligibilityLabel)}</em><small class="people-aircraft-eligibility-copy">${esc(eligibilitySummary)}</small></div>
