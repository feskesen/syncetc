...
${esc(JSON.stringify(app,null,2))}</pre>`:''}</main>`;
  ...
  async function sendPortalInvite(){ if(!state.selected) return; await runButton('at-send-portal-invite','Sending…', async()=>{ const res=await accessCall({action:'organization_send_applicant_portal_invite', organization_id:state.orgId, application_id:state.selected.application_id, redirect_to: location.origin + '/applicant-portal'}); state.message='Applicant portal link sent.'; state.messageKind='ok'; if(res.applicant){ state.selected=res.applicant; const ix=state.allApplicants.findIndex(a=>a.application_id===res.applicant.application_id); if(ix>=0) state.allApplicants[ix]=res.applicant; state.applicants=visibleApplicants(); } render(); }); }
