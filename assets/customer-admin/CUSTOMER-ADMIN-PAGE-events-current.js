...
(function () {
  ...
  function formHtml() {
    ...
    const contentBody = `<label class="events-field">Short summary<textarea class="events-textarea" id="event-summary">${esc(ev.summary || "")}</textarea></label><label class="events-field">Full description / notes<textarea class="events-textarea" id="event-description">${esc(ev.description || "")}</textarea></label>`;
  ...
  function bindDraftReminderControls() {
    ...
    const panel = document.querySelector(".events-control-panel");
    const callout = document.getElementById("events-draft-callout");
    const sync = () => {
      const draft = status && status.value === "draft";
      if (panel) panel.classList.toggle("draft-alert", !!draft);
      if (callout) callout.hidden = !draft;
    };
